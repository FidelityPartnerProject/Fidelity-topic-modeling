...
fund$FUND_STRATEGY <- sapply(fund$FUND_STRATEGY, function(x , pattern = "<br>"){gsub(pattern, " ", x)})
fund$FUND_STRATEGY <- sapply(fund$FUND_STRATEGY, function(x , pattern = "\r\r\n"){gsub(pattern, " ", x)})
...
cleanf <- fund %>% mutate(fund_id = row_number()) %>% distinct(TCKR_C, FUND_STRATEGY,.keep_all = T) %>% select(fund_id,everything())

strategy_words <- cleanf %>% unnest_tokens(word, FUND_STRATEGY) %>% select(fund_id, word)

tidy_strategy_words <- strategy_words %>% anti_join(stop_words)

#  word frequency in each document
## most common words 
tidy_strategy_words %>% count(fund_id,word, sort = TRUE) 


tidy_strategy_words %>% count(fund_id, word, sort = TRUE) %>% filter(n > 15) %>%
...
lemmatization <- tidy_strategy_words %>% mutate(lemma = word%>% lemmatize_words())
...
# term frequency of "fund" increases by approximately 1000
```
...
```{r ticker tf-idf}
# tf
ticker_words<- lemmatization %>% count(fund_id, lemma, sort = TRUE) 
